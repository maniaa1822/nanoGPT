"""
Prepare the complex-csm dataset for character-level language modeling.
Maps characters to ints and writes train.bin, val.bin, and meta.pkl.

Input discovery order (first existing is used):
1) data/complex_csm/input.txt
2) data/complex_csm/complex-csm.dat
3) transCSSR/data/complex-csm.dat (repo-relative)
"""
import os
import pickle
import numpy as np
...
def find_input_path() -> str:
    ...
    candidates = [
        ...
        os.path.join(here, "complex-csm.dat"),
        os.path.join(repo_root, "transCSSR", "data", "complex-csm.dat"),
    ]
    for path in candidates:
        if os.path.exists(path):
            return path
    raise FileNotFoundError(
        "No input file found. Place your sequence into data/complex_csm/input.txt or complex-csm.dat, "
        "or ensure transCSSR/data/complex-csm.dat exists."
    )
